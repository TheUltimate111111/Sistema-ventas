<?php
    ...
    try {
        ...
        $ventaStmt = $pdo->prepare('INSERT INTO ventas (cliente_id, usuario_id, subtotal, iva, total_factura, monto_pagado, cambio, fecha_emision) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())');
        $cambio = round($montoPagado - $total, 2);
        $ventaStmt->execute([$clienteId, (int)$usuario['id'], $subtotal, $iva, $total, $montoPagado, $cambio]);
